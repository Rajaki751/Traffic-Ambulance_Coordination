  ...
  List<TrafficPoint> _generateSyntheticKathmanduTraffic() {
    ...
    for (var i = 0; i < cellCount; i++) {
      for (var j = 0; j < cellCount; j++) {
        final lat = KathmanduLocation.centerLat +
            (i - cellCount / 2) * (radiusKm / 110.0);
        final lon = KathmanduLocation.centerLon +
            (j - cellCount / 2) *
                (radiusKm /
                    (110.0 * cos(KathmanduLocation.centerLat * pi / 180)));
        final distKm = sqrt(pow(lat - KathmanduLocation.centerLat, 2) +
                pow(lon - KathmanduLocation.centerLon, 2)) *
            110.0;
        final base = (1.0 - (distKm / radiusKm)).clamp(0.0, 1.0);
        final hour = DateTime.now().hour;
        final peak =
            (hour >= 7 && hour <= 9) || (hour >= 16 && hour <= 19) ? 0.25 : 0.0;
